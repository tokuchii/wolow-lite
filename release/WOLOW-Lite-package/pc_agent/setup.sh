...
echo ""
grep -oP '(?<=token: ).+' config.yaml 2>/dev/null || python3 -c "import yaml; print('  ' + yaml.safe_load(open('config.yaml'))['server']['token'])" 2>/dev/null
echo ""
echo "  ============================================"
echo ""
...
echo ""
echo "  Phone app: enter your PC's IP and the token above."
echo "  Note: audio device switching requires Windows (svcl.exe)."
echo "  Other features (WoL, reboot, etc.) work on all platforms."
echo ""
echo "  ============================================"
echo ""
